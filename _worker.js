...
/*
 * Project Hit (هیت) - IoT Device Telemetry Gateway
...
const SYSTEM_DEFAULTS = {
    ...
    isPaused: false,
    silentAlerts: false,
    githubRepo: "hosseinit1988/Auto-Workers-Cloudflare-Prxoy",
...
function generateApiKey(name) {
    ...
    const raw = `hit_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
    const key = raw;
    return {
        id,
        ...
        try {
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    const dashboardUrl = env.DASHBOARD_URL || 'https://raw.githubusercontent.com/hosseinit1988/Auto-Workers-Cloudflare-Prxoy/main/dashboard.html';
                    try {
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (isValidUser) {
                            const subscriptionUrl = env.SUBSCRIPTION_URL || 'https://raw.githubusercontent.com/hosseinit1988/Auto-Workers-Cloudflare-Prxoy/main/subscription.html';
                            try {
    ...
    async scheduled(event, env, ctx) {
        try {
            ...
            if (sysConfig.autoUpdate && sysConfig.cfAccountId && sysConfig.cfApiToken && sysConfig.cfWorkerName) {
                const repo = (sysConfig.githubRepo || "hosseinit1988/Auto-Workers-Cloudflare-Prxoy")
...
function obfuscateCode(srcText) {
    ...
    cleanCode = cleanCode.replace(/export\s+default\s+/g, "const _0xHitModule = ");
    cleanCode += "\nreturn _0xHitModule;";
    ...
    const finalLoaderCode =
        rawImportsStr +
        "\n\n" +
        "// Hit Gateway - Obfuscated Loader Context (v2.5.4.2 Optimized)\n" +
        'const _0xHitPayload = "' +
        hexOutput +
        '";\n' +
        "const _0xHitKey = " +
        randKey +
        ";\n\n" +
        "const _0xHitBytes = new Uint8Array((_0xHitPayload.match(/.{1,2}/g) || []).map(x => parseInt(x, 16) ^ _0xHitKey));\n" +
        "const _0xHitCode = new TextDecoder().decode(_0xHitBytes);\n" +
        "const _0xHitRuntime = new Function(" +
        bindingNames.map((name) => '"' + name + '"').join(", ") +
        ", _0xHitCode)(" +
        bindingNames.join(", ") +
        ");\n\n" +
        "export default _0xHitRuntime;";
